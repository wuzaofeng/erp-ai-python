...
def append_user_message(user_id: str, content: str, conversation_id: str = "") -> None:
    ...
    with conn:
        conn.execute(
            ...
            (user_id, conversation_id, "user", content, time.time()),
        )
    conn.close()
    _trim(user_id, conversation_id)


def append_assistant_message(user_id: str, content: str, conversation_id: str = "") -> None:
    conn = get_conn()
    with conn:
        conn.execute(
            "INSERT INTO conversations(user_id, conversation_id, role, content, created_at) VALUES(?,?,?,?,?)",
            (user_id, conversation_id, "assistant", content, time.time()),
        )
    conn.close()
    _trim(user_id, conversation_id)
    history_len = len(get_history(user_id, conversation_id))
    logger.info("Memory", f"对话历史已更新 | userId={user_id} | convId={conversation_id} | 当前轮数={history_len // 2}")
