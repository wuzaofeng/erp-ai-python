...
def build_system_prompt(
    ...
    web_search_rule = (
        "6. 工具选择规则：根据问题性质选择正确工具。"
        "→ 用户问的是公司内部业务数据（订单/库存/采购/财务/员工等）：使用 ERP 查询工具。"
        "→ 用户问的是外部公开信息（新闻/体育赛事与球员数据/天气/汇率/股价/政策法规/行业动态等非公司内部的事情）：使用 web_search 工具，不得以'无法提供'拒绝。"
        "→ 混合场景（如'本月采购额与同期行业均价对比'）：先查 ERP，再用 web_search 补充外部数据。"
        "判断依据：这个问题的答案存在于公司 ERP 数据库中吗？是 → ERP 工具；否 → web_search。"
