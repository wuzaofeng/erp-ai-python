...
def _lookup_catalog_params(table_name: str) -> tuple[str | None, dict | None, str]:
    """根据 tableName 从 erp_form_catalog 查出配置的 api_path / extra_body / body_mode"""
    try:
        from db import get_conn
        form_code = table_name.split(".")[0]
        conn = get_conn()
        row = conn.execute(
            "SELECT api_path, extra_body, body_mode FROM erp_form_catalog WHERE form_code = ? AND enabled = 1",
            (form_code,),
        ).fetchone()
        conn.close()
        if not row:
            return None, None, "merge"
        api_path  = row["api_path"] or None
        body_mode = row["body_mode"] or "merge"
        extra_body = None
        if row["extra_body"]:
            try:
...
class FilterItem(BaseModel):
    ...
    Logic: Optional[str] = Field(default=None, description="当前条件与下一条件的逻辑关系：and（且）或 or（或）；最后一条无下一条，不填")
...
def create_common_query_tool(erp_cookie: str, erp_auth: str, user_id: str = "") -> StructuredTool:
    ...
    async def common_query_handler(
        ...
        # apiPath / extraBody / bodyMode 完全由 DB 目录决定，忽略 AI 传参
        catalog_api_path, catalog_extra_body, catalog_body_mode = _lookup_catalog_params(tableName)
        ...
        params = {
            ...
            "apiPath":   catalog_api_path,
            "extraBody": catalog_extra_body or {},
            "bodyMode":  catalog_body_mode,
        }
