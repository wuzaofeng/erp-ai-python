...
def _lookup_catalog_params(table_name: str) -> tuple[str | None, dict | None]:
    """根据 tableName 从 erp_form_catalog 查出配置的 api_path / extra_body"""
    try:
        from db import get_conn
        form_code = table_name.split(".")[0]
        conn = get_conn()
        row = conn.execute(
            "SELECT api_path, extra_body FROM erp_form_catalog WHERE form_code = ? AND enabled = 1",
            (form_code,),
        ).fetchone()
        conn.close()
        if not row:
...
def create_common_query_tool(erp_cookie: str, erp_auth: str, user_id: str = "") -> StructuredTool:
    ...
    async def common_query_handler(
        ...
        params = {
            ...
            "apiPath": resolved_api_path,
            "extraBody": resolved_extra_body,
        }
